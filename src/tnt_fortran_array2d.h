/* $Id: tnt_fortran_array2d.h,v 1.3 2006-10-30 15:17:55 gmurphy Exp $  */
/*
*
...
template < class T > Fortran_Array2D < T >::Fortran_Array2D ():v_ (), m_ (0), n_ (0),
    data_ (0)
  {
  }
  ...
  template < class T >
    Fortran_Array2D < T >::Fortran_Array2D (const Fortran_Array2D < T >
					    &A):v_ (A.v_), m_ (A.m_),
    n_ (A.n_), data_ (A.data_)
  {
  }
  ...
  template < class T >
    Fortran_Array2D < T >::Fortran_Array2D (int m, int n):v_ (m * n), m_ (m),
    n_ (n), data_ (v_.begin ())
  {
  }
  ...
  template < class T >
    ...
    T >::inject (const Fortran_Array2D & A)
  {
    if (m_ == A.m_ && n_ == A.n_)
  ...
  template < class T >
    Fortran_Array2D < T > &Fortran_Array2D < T >::ref (const Fortran_Array2D <
						       T > &A)
  {
    if (this != &A)
      {
  ...
  template < class T >
    Fortran_Array2D < T > &Fortran_Array2D <
    T >::operator= (const Fortran_Array2D < T > &A)
  {
    return ref (A);
  }

  template < class T > inline int Fortran_Array2D < T >::dim1 () const
  {
    return m_;
  }

  template < class T > inline int Fortran_Array2D < T >::dim2 () const
  {
    return n_;
  }
