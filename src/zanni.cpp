...
/*
	...
	*/
	{
	Psi[ii][jj] = (4.0/3.0)*(bz0*r0*r0)*pow(r/r0, 0.75)*pow(r,1.25)*pow(m, 1.25)/pow((m*m*r2 + z2), 5.0/8.0); 
	Psi[ii][jj] =  (4.0/3.0)*(bz0*r0*r0)*pow(r/r0, 0.75)*pow(m, 1.25)/pow((m*m + z2/r2), 5.0/8.0); 
	//Psi[ii][jj] =  r2;
	}
  ...
  for (int jj = 2; jj < ny; jj++)
	...
	*/
	...
	if (z > h)
		{
			double rhoa0=1e-4;
			//rho = rhoa0*pow(r0/sphere_r, gammam1i) ;
			double gamp= PhysConsts::gamma/gammam1;
			double gampi= 1/gamp;
			double GM = 1/(eps*eps);
			pressure=rhoa0 * gampi * GM * pow(r0/sphere_r , gamp)/ r0; 
