 ...
 */
  ...
  */
  ...
  for (int i = 2; i < nx - 1; ++i)
	...
	if (  delpz + delpr > alphaB* min4 ( pressurearr[i  ][j  ] , pressurearr[i+1][j  ] , pressurearr[i  ][j+1] , pressurearr[i+1][j+1] )) 
	{
		swa=true;
	}
	double betaB=0.5;
	...
	double b21 = mesh[i][j] _B_X * mesh[i][j] _B_X + mesh[i][j] _B_Y * mesh[i][j] _B_Y + mesh[i][j] _B_Z * mesh[i][j] _B_Z ;
	double b22 = mesh[i+1][j] _B_X * mesh[i+1][j] _B_X + mesh[i+1][j] _B_Y * mesh[i+1][j] _B_Y + mesh[i+1][j] _B_Z * mesh[i+1][j] _B_Z ;
	double b23 = mesh[i][j+1] _B_X * mesh[i][j+1] _B_X + mesh[i][j+1] _B_Y * mesh[i][j+1] _B_Y + mesh[i][j+1] _B_Z * mesh[i][j+1] _B_Z ;
	double b24 = mesh[i+1][j+1] _B_X * mesh[i+1][j+1] _B_X + mesh[i+1][j+1] _B_Y * mesh[i+1][j+1] _B_Y + mesh[i+1][j+1] _B_Z * mesh[i+1][j+1] _B_Z ;
...
#define SADSTORY
		...
		<< " at (" << i << "," << j << ")" << std::endl;
			...
			double g1= 0.5*(mesh[i][j] _MOMX* mesh[i][j] _MOMX + mesh[i][j] _MOMY* mesh[i][j] _MOMY + mesh[i][j] _MOMZ* mesh[i][j] _MOMZ )/
			mesh[i][j] _MASS;
			double g2= 0.5*(mesh[i][j] _B_X* mesh[i][j] _B_X + mesh[i][j] _B_Y* mesh[i][j] _B_Y + mesh[i][j] _B_Z* mesh[i][j] _B_Z );
