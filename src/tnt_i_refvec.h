/* $Id: tnt_i_refvec.h,v 1.3 2006-10-30 15:17:55 gmurphy Exp $  */
/*
*
...
template < class T > i_refvec < T >::i_refvec ():data_ (NULL),
    ref_count_ (NULL)
  {
  }
...
template < class T > i_refvec < T >::i_refvec (T * data):data_ (data),
    ref_count_
    (NULL)
  {
  }

  template < class T > inline T * i_refvec < T >::begin ()
  {
    return data_;
  }
  ...
  template < class T > inline T & i_refvec < T >::operator[] (int i)
  {
    return data_[i];
  }


  template < class T > inline const T *i_refvec < T >::begin () const
  {
    return data_;
  }
