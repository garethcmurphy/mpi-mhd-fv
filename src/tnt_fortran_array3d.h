/* $Id: tnt_fortran_array3d.h,v 1.3 2006-10-30 15:17:55 gmurphy Exp $  */
/*
*
...
template < class T > Fortran_Array3D < T >::Fortran_Array3D ():v_ (), m_ (0), n_ (0), k_ (0),
    data_ (0)
  {
  }
  ...
  template < class T >
    Fortran_Array3D < T >::Fortran_Array3D (const Fortran_Array3D < T >
					    &A):v_ (A.v_), m_ (A.m_),
    n_ (A.n_), k_ (A.k_), data_ (A.data_)
  {
  }
  ...
  template < class T >
    Fortran_Array3D < T >::Fortran_Array3D (int m, int n,
					    int k):v_ (m * n * k), m_ (m),
    n_ (n), k_ (k), data_ (v_.begin ())
  {
  }
  ...
  template < class T >
    Fortran_Array3D < T >::Fortran_Array3D (int m, int n, int k,
					    const T & val):v_ (m * n * k),
    m_ (m), n_ (n), k_ (k), data_ (v_.begin ())
  {
  ...
  template < class T >
    inline const T & Fortran_Array3D < T >::operator  () (int i, int j,
							  int k) const
  {
#ifdef TNT_BOUNDS_CHECK
    assert (i >= 1);
  ...
  template < class T >
    ...
    T >::inject (const Fortran_Array3D & A)
  {
  ...
  template < class T >
    Fortran_Array3D < T > &Fortran_Array3D < T >::ref (const Fortran_Array3D <
						       T > &A)
  {
  ...
  template < class T >
    Fortran_Array3D < T > &Fortran_Array3D <
    T >::operator= (const Fortran_Array3D < T > &A)
  {
    return ref (A);
  }

  template < class T > inline int Fortran_Array3D < T >::dim1 () const
  {
    return m_;
  }

  template < class T > inline int Fortran_Array3D < T >::dim2 () const
  {
    return n_;
  }

  template < class T > inline int Fortran_Array3D < T >::dim3 () const
  {
    return k_;
  }
