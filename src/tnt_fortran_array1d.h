/* $Id: tnt_fortran_array1d.h,v 1.3 2006-10-30 15:17:55 gmurphy Exp $  */
/*
*
...
template < class T > Fortran_Array1D < T >::Fortran_Array1D ():v_ (), n_ (0),
    data_ (0)
  {
  }

  template < class T >
    Fortran_Array1D < T >::Fortran_Array1D (const Fortran_Array1D < T >
					    &A):v_ (A.v_), n_ (A.n_),
    data_ (A.data_)
  {
#ifdef TNT_DEBUG
  ...
  template < class T >
    Fortran_Array1D < T >::Fortran_Array1D (int n):v_ (n), n_ (n),
    data_ (v_.begin ())
  {
#ifdef TNT_DEBUG
  ...
  template < class T >
    Fortran_Array1D < T >::Fortran_Array1D (int n, const T & val):v_ (n),
    n_ (n), data_ (v_.begin ())
  {
#ifdef TNT_DEBUG
  ...
  template < class T >
    Fortran_Array1D < T >::Fortran_Array1D (int n, T * a):v_ (a), n_ (n),
    data_ (v_.begin ())
  {
#ifdef TNT_DEBUG
  ...
  template < class T >
    ...
    T >::inject (const Fortran_Array1D & A)
  {
    if (A.n_ == n_)
  ...
  template < class T >
    Fortran_Array1D < T > &Fortran_Array1D < T >::ref (const Fortran_Array1D <
						       T > &A)
  {
    if (this != &A)
      {
  ...
  template < class T >
    Fortran_Array1D < T > &Fortran_Array1D <
    T >::operator= (const Fortran_Array1D < T > &A)
  {
    return ref (A);
  }

  template < class T > inline int Fortran_Array1D < T >::dim1 () const
  {
    return n_;
  }

  template < class T > inline int Fortran_Array1D < T >::dim () const
  {
    return n_;
  }
...
/* ............................ exented interface ......................*/
  ...
  template < class T >
    inline Fortran_Array1D < T > Fortran_Array1D < T >::subarray (int i0,
								  int i1)
  {
#ifdef TNT_DEBUG
